define(['lodash', 'core/eventifier', 'core/providerRegistry', 'taoTests/runner/dataHolder'], function (_, eventifier, providerRegistry, dataHolderFactory) { 'use strict';
    ...
    dataHolderFactory = dataHolderFactory && dataHolderFactory.hasOwnProperty('default') ? dataHolderFactory['default'] : dataHolderFactory;
    ...
    function testRunnerFactory(providerName, pluginFactories, config) {
      ...
      runner = eventifier({
         ...
         */
        init: function init() {
          var self = this; //instantiate the plugins first
          ...
          providerRun('install').then(_.partial(providerRun, 'loadPersistentStates')).then(_.partial(pluginRun, 'install')).then(_.partial(providerRun, 'init')).then(_.partial(pluginRun, 'init')).then(function () {
            self.setState('init', true).off('init.internal').after('init.internal', function initDone() {
              this.render();
            }).trigger('init');
          }).catch(reportError);
          return this;
        ...
        loadItem: function loadItem(itemRef) {
          ...
          providerRun('loadItem', itemRef).then(function (itemData) {
            self.setItemState(itemRef, 'loaded', true).off('loaditem.internal').after('loaditem.internal', function loadItemDone() {
          ...
          providerRun('destroy').then(function () {
            pluginRun('destroy').then(function () {
              ...
              return destroyed.then(function () {
                self.setTestContext({}).setTestMap({}).setState('destroy', true).trigger('destroy');
              });
            }).catch(reportError);
          }).catch(reportError);
          return this;
