...
class TAOTsaveContent
{
	...
	function getOutput($ressource)
	{
		...
		if(isset($ressource["tao:showprogressbar"])){
			if ($ressource["tao:showprogressbar"]=="on") {
				$params.='<progressmeter id="test_progressmeter" left="'.$ressource["progressbarleft"].'" top="'.$ressource["progressbartop"].'" mode="determined" value="0"/>';
			}
		}
		if(isset($ressource["tao:showlistbox"])){
			if ($ressource["tao:showlistbox"]=="on") {
				$params.='<listbox id="testItems_listbox" left="10" top="10" height="400" width="100"/>';
			}
		}
		$labelandcomment ='';
		if (isset($ressource["tao:showLabel"])) {
			$labelandcomment .='<label id="testLabel_label" left="10" top="10" class="Label" value="&lt;b&gt;&lt;u&gt;#{XPATH(/tao:TEST/rdfs:LABEL)}#&lt;/u&gt;&lt;/b&gt;" style=\'\'/>';
		}
		if (isset($ressource["tao:showComment"])) {
			$labelandcomment .='<label id="testComment_label" left="25" top="30" class="Comment" value="#{XPATH(/tao:TEST/rdfs:COMMENT)}#"/>';
		}
		if ( (!(isset($ressource["urlleft"]))) or ($ressource["urlleft"]=="") ) {
			$ressource["urlleft"]="http://www.tao.lu/middleware/itempics/default/left.swf";
		}
		if ( (!(isset($ressource["urlright"]))) or ($ressource["urlright"]=="") ) {
		...
		if ((!(isset($ressource["navtop"])))or($ressource["navtop"]=="")) {
			$ressource["navtop"]="400";
		}

		//default left position for back, next and progress bar
		if ((!(isset($ressource["navleft"]))) or ($ressource["navleft"]=="")) {
			$ressource["navleft"]="200";
		}
		...
		//with the default FR and DE button
		...
		*/
		...
		$xul.='<tao:TESTPRESENTATION>
			<xul>
				<stylesheet id="test_stylesheet" src="./test.css" />
				<box id="testContainer_box">'.$labelandcomment.
					'<box id="itemContainer_box" left="100" top="65"/>
						'.$backbutton.'
						 <button id="nextItem_button" left="'.$ressource["navleftnext"].'" url="'.$ressource["urlright"].'" top="'.$ressource["navtop"].'" label="Next" image="item_next.jpg" disabled="true" oncommand="tao_test.nextItem"/>'.
						 $params.'
	        	</box>
			</xul>
		...
		*/
		...
		if  ($ressource["TAO:HALTCRITERIA"]=="DELTASCORE") {
			$haltcriteriaoutput='<tao:HALTCRITERIA max="'.$ressource["max"].'" value="DELTASCORE">
			<tao:PARAM name="threshold" value="'.$ressource["deltascorethreshold"].'"/>
			</tao:HALTCRITERIA>';
		}
		if  ($ressource["TAO:HALTCRITERIA"]=="DELTASE") {
			$haltcriteriaoutput='<tao:HALTCRITERIA value="DELTASE">
			<tao:PARAM name="threshold" value="'.$ressource["deltascorethreshold"].'"/>
			</tao:HALTCRITERIA>';
		}
		...
		$delay="";
		if (isset($ressource["delay"]) and ($ressource["delay"]!="")) {
			$delay='delay="'.$ressource["delay"].'" delaypolicy="sequential" firstselection="montecarlo"';
		}
		...
		$xml="<?xml version='1.0' encoding='UTF-8' ?>
				<tao:TEST xmlns:rdf='http://www.w3.org/1999/02/22-rdf-syntax-ns#' rdf:ID=\"".$script."\" xmlns:tao='http://www.tao.lu/tao.rdfs#' xmlns:rdfs='http://www.w3.org/TR/1999/PR-rdf-schema-19990303#'>
					<rdfs:LABEL lang=\"".$lang."\">".$label."</rdfs:LABEL>
					<rdfs:COMMENT lang=\"".$lang."\">".$comment."</rdfs:COMMENT>".
					$xul."	
					<tao:PASSWORD>".$ressource["tao:password"]."</tao:PASSWORD>
					<tao:DURATION>".$ressource["tao:duration"]."</tao:DURATION>
					<tao:HASSEQUENCEMODE ".$delay." >".$ressource["TAO:HASSEQUENCEMODE"]."</tao:HASSEQUENCEMODE>
					...
					<tao:HASSCORINGMETHOD ".$scoringmethodsparams.">".$ressource["TAO:HASSCORINGMETHOD"]."</tao:HASSCORINGMETHOD>
					<tao:TESTLISTENERS></tao:TESTLISTENERS>
					<tao:CUMULMODEL>".$ressource["TAO:CUMULMODEL"]."</tao:CUMULMODEL>".
					$haltcriteriaoutput.
					$tresholds;
		...
		if (isset($ressource["TAO:CITEM"])){
			...
			foreach ($ressource["TAO:CITEM"] as $a=>$b){	//$i=$i+1;
				if ( (isset($b["Sequence"])) and ($b["Sequence"]!="") and ($b["Sequence"]!="0") ){
					$index=$b["Sequence"];
					$b["key"]=$a;
					$x[$index]=$b;
				} 
